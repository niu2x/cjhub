---
name: alexis
description: 仅在用户明确点名 alexis 时触发。使用快捷指令执行结构化需求管理：alexis-new 新建需求并澄清、alexis-plan 起草或重规划执行方案、alexis-review 评审或复评执行计划、alexis-auto-plan 自动迭代规划评审直到通过、alexis-apply-step 执行单个任务、alexis-apply 串行执行所有未完成任务、alexis-accept 最终验收。全流程要求文档落库与会话独立，不依赖当次对话上下文。
---
...
4. `alexis-auto-plan <需求ID>`
5. `alexis-apply-step <需求ID> <TaskID>`
6. `alexis-apply <需求ID>`
7. `alexis-accept <需求ID>`
...
2. **按需调用**：根据需求性质选择合适技能：
   - `alexis-new` 澄清需求时：若需求涉及特定领域，调用相关技能获取专业视角
   - `alexis-plan` 制定方案时：参考相关技能的最佳实践和工作流
   - `alexis-review` 评审时：利用相关技能的规范进行专业检查
   - `alexis-apply-step` 执行时：直接委派给相关技能执行具体任务
...
3. 按指令读取：
   ...
   - `alexis-auto-plan` → `references/templates/template-auto-plan.md`
   - `alexis-apply-step` → `references/templates/template-apply.md`
   - `alexis-apply` → `references/templates/template-apply-all.md`
...
| alexis-auto-plan | 自动迭代 plan-review 循环，直到评审通过 |
| alexis-apply-step | 执行单个 Task，强制校验前序已完成 |
| alexis-apply | 串行执行所有未完成 Task，每个 Task 在子 Agent 中执行 |
